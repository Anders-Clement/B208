...
for i = 1:tooltop.InstructionCount()
    ...
    if pose ~= -1 
        ...
        temp(1,4:7) = rotm2quat(matrix(1:3,1:3));
...
for i=1:length(values)
    ...
    for j=1:size(time,1)
        ...
        if values(i) < time(j,2)
            ...
            cords(counter, 1:7) = a1 + a2*timeIntoPeriode + a3*timeIntoPeriode^2 + a4*timeIntoPeriode^3;
            counter = counter + 1;
            break;
        end
    end
...
plot(valuesTemp,cords(:,4:7))
figure();
plot(valuesTemp,cords(:,1:3))
...
urMoveJ(sock, Angles{1,1});

urChangeVel(sock, [0.2,0.2]); %Setting the acc and vel high because we're rebels.
...
for idx=1 : length(values)
   ...
   robot.MoveL(rad2deg(AnglesTemp));
   urMoveJ(sock, Angles{idx,1});
